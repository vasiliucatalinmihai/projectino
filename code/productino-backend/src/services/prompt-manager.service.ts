...
export class PromptManagerService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    try {
      ...
      this.logger.error(`Prompt sync failed: ${error?.message}`, error?.stack);
    }
  }
  ...
  async syncFromFiles(): Promise<void> {
    ...
    for (const file of files) {
      ...
      const { key: _key, description, ...config } = data;
  ...
  async refreshCache(): Promise<void> {
    ...
    for (const prompt of await this.prompts.findAll()) {
      ...
      nextCache.set(prompt.key, {
        ...
        config: (version.config ?? {}) as PromptConfig,
        render: Handlebars.compile(version.content, { noEscape: true }),
      });
    }
